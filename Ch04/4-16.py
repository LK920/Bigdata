...
date : 20/08/13
name : kang
content : 머신러닝 - 신경망 실습하기
"""

import numpy as np
from scipy.special import expit as sigmoid
#sigmoid 활성화 함수(탄젠트 h 함수) 역할은 신호가 끊어지지 않게 증폭해준다.
...
class NeuralNetwork:
    ...
    def __init__(self, inputnodes, hiddennodes, outputnodes, learningrate):
        ...
        #가중치 초기화(wih: input에서 hidden으로 가는 가중치, who: hidden에서 output으로 가는 가중치)
    ...
    def fit(self, input_list, target_list):
        ...
        #입력 데이터를 2차원 행렬로 변환(ndmin=2 : 차수 2, .T : 전치)
        inputs = np.array(input_list, ndmin=2).T
        target = np.array(target_list, ndmin=2).T

        #은닉계층으로 돌아오는 데이터 계산(dot:행렬곱)
